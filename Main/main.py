...
class Tool:
    ...
    def rulewerk(self):
        ...
        # call function to write bencmarking results to csv file
        self.write_benchmark_results(timestamp, "random_taskname_for_now", "Rulewerk", execution_time, memory_info)  # TODO add count of grounded atoms


    def get_rls_file_paths(self, directory):
        rls_file_paths = []
    ...
    def main(self):
        ...
        parser.add_argument('--solver', required=True, type=str, choices=['clingo', 'nemo', 'rulewerk', 'souflle', 'all'])
        parser.add_argument('--input_dir', type=str, required=True)

        args = parser.parse_args()

        rule_file_path = args.input_dir
        rls_files = self.get_rls_file_paths(rule_file_path)
    ...
    def write_benchmark_results(self, timestamp, task, tool, execution_time, memory_info):
